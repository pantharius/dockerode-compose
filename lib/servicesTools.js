...
module.exports = {
  ...
  buildHostConfig: function (projectName, service, recipe, cwd) {
    ...
    if (service.volumes_from !== undefined) {
      for (let volume_from of service.volumes_from) {
        let vf = volume_from.split(':');
        let svf = recipe.services[vf[0]];
    ...
    if (service.sysctls !== undefined) {
      if (service.sysctls.length > 0) {
        let sysctls = {};
        for (let sysctlsb of service.sysctls) {
    ...
    if (service.tmpfs !== undefined) {
      ...
      if (Array.isArray(service.tmpfs)) {
        for (let tmpfsb of service.tmpfs) {
          let p = tmpfsb.split(':');
          if (p[1] === undefined) {
            p[1] = '';
          }
  ...
  buildVolumesHostconfig: function (projectName, volumes, output, type, cwd, recipe) {
    ...
    for (let volume of volumes) {
      if (typeof volume === 'string' || volume instanceof String) {
        if (
          ...
          let stdVolume = standardizeVolumeStr(volume, cwd);
          if(volume.substring(0, 2) === './'){
            parts = stdVolume.split(':')
            stdVolume=join(global.__dirname + parts[0].substring(2)) + ":" + parts[1];
          }
          let aux = stdVolume;
          if (type == 'ro') {
            aux += ':ro';
          ...
          let aux = projectName + '_' + volume;
          let vf = volume.split`:`;
          if(recipe && recipe.volumes && vf.length==2 && recipe.volumes[vf[0]] && recipe.volumes[vf[0]].external===true)
            aux=volume;
          if (type == 'ro') {
            aux += ':ro';
          }
  ...
  buildUpParams: function (service, upParams){
    service = this.buildUpParamsPorts(service, upParams)
    service = this.buildUpParamsEnvVars(service, upParams);
    return service;
  },
  buildUpParamsPorts: function (service, upParams){
    const servicePortsParts = service.ports.map(p=>p.split(':'));
    upParams.ports.forEach(paramPort => {
      let paramPortParts = paramPort.split(':');
      let servicePortIndex = servicePortsParts.findIndex(c=>c[1]==paramPortParts[1]);
      if(servicePortIndex!=-1)
        servicePortsParts[servicePortIndex][0]=paramPortParts[0];
    });
    service.ports = servicePortsParts.map(c=>c.join(':'))
    return service;
  },
  buildUpParamsEnvVars: function (service, upParams){
    if (upParams.environment !== undefined) {
      const upParamEnv = upParams.environment[service.name];
      if (upParamEnv !== undefined) {
        if (Array.isArray(upParamEnv)) {
          for (const environment_line of upParamEnv) {
            const envParts = environment_line.split('=');
            const envName = envParts[0];
            if(Array.isArray(service.environment)){
              let resIndex = service.environment.map(c=>c.split('=')).findIndex(c=>c[0]==envName);
              if(resIndex>=0) service.environment[resIndex]=environment_line;
              else service.environment.push(environment_line);
            }else{
              service.environment[envName]=envParts[1];
            }
          }
        } else {
          const envsNames = Object.keys(upParamEnv);
          for (const envName of envsNames) {
            const envValue = upParamEnv[envName];
            if(Array.isArray(service.environment)){
              let resIndex = service.environment.map(c=>c.split('=')).findIndex(c=>c[0]==envName);
              if(resIndex>=0) service.environment[resIndex]=envName+"="+envValue;
              else service.environment.push(envName+"="+envValue);
            }else{
              service.environment[envName]=envValue;
  ...
  buildEnvVars: function (service) {
    ...
    if (service.environment !== undefined) {
      if (Array.isArray(service.environment)) {
        ...
        let envsNames = Object.keys(service.environment);
  ...
  buildNetworks: function (projectName, serviceName, serviceNetworks, networksToAttach, opts, recipe) {
    if (Array.isArray(serviceNetworks)) {
      for (let index = 0; index < serviceNetworks.length; index++) {
        let networkName = serviceNetworks[index];
        if(!recipe.networks
            || !recipe.networks[networkName]
            || recipe.networks[networkName].external !== true){
  ...
  buildDockerImage: async function (
    ...
    if (dockerfile !== null) {
      ...
      let tarStream = tar.pack(buildPath);
